...
func getImplementedRequirements(projectId string) []requests_models.ImplementedRequirement {
	...
	partyInfo := component_user_party.GetComponentUserParty(int64(id))
	compToRespPartyMap := make(map[string]map[string][]string, 0)
	for _, info := range partyInfo {
		if _, exists := compToRespPartyMap[info.ComponentId]; !exists {
			compToRespPartyMap[info.ComponentId] = make(map[string][]string, 0)
		}
		if _, exists := compToRespPartyMap[info.ComponentId][info.UserId]; !exists {
			compToRespPartyMap[info.ComponentId][info.UserId] = make([]string, 0)
		}
		compToRespPartyMap[info.ComponentId][info.UserId] = append(
			compToRespPartyMap[info.ComponentId][info.UserId],
			info.PartyId,
		)
	}
	...
	for controlID, statementIDs := range controlToStatementMap {
		...
		for _, statementID := range statementIDs {
			...
			for _, c := range components {
				rps := make([]requests_models.RolePartyMap, 0)
				for userID, partyIDs := range compToRespPartyMap[c] {
					rps = append(rps, requests_models.RolePartyMap{
						UserUUID:   userID,
						PartyUUIDs: partyIDs,
					})
				}
...
func main() {
	...
	r.GET("/information/get-user/:uuid", func(c *gin.Context) {
		id := c.Param("uuid")
		user := information.GetUser(id)
